...
- **Precomputing a full pattern matrix** — all word-guess feedback patterns
  are calculated once and stored, eliminating redundant computation during solving
- **Entropy-based pruning** — at each step, only the guess with the highest
  information gain is chosen, minimizing the expected number of remaining candidates
